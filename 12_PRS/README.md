...
| File | Description |
|---|---|
| `PRS-CS.sh` | Infers posterior SNP effect estimates from the DEP-Meng-TPMI training GWAS summary statistics with PRS-CS (high-dimensional Bayesian regression with continuous shrinkage priors; https://github.com/getian107/PRScs), per chromosome 1-22. EAS LD reference panel based on UK Biobank data (provided by the PRS-CS developers, `ldblk_ukbb_eas`). Input sumstats: `sorted_meta_mdd2023diverse_EAS_TPMI-mdd.Diff-Model.I85.allele_aligned.forPRS-CS.txt` (allele-aligned to the reference, N = 87,216). |
