#!/usr/bin/env bash
# =============================================================================
# Quality control of raw genotyping data (PLINK v1.9)
# =============================================================================
...
#
# NOTE: all PLINK analyses used v1.9, except the dosage-based association
...
#     (X chromosome coded as chr 23 in the BIM file)
plink --bfile ${RAW} \
      --check-sex \
      --out ${OUT}.sexcheck
...
plink --bfile ${RAW} --remove ${OUT}.sexremove.txt --make-bed --out ${OUT}.step1

# (2) Missingness > 3% and heterozygosity beyond +/- 6 SD from the mean (n=28)
plink --bfile ${OUT}.step1 --mind 0.03 --make-bed --out ${OUT}.step2a
plink --bfile ${OUT}.step2a --het --out ${OUT}.het
Rscript -e '
...
'
plink --bfile ${OUT}.step2a --remove ${OUT}.hetremove.txt --make-bed --out ${OUT}.step2b

# (3) Duplicates / related individuals by IBD (PI_HAT > 0.1875; n=168)
plink --bfile ${OUT}.step2b \
      --indep-pairwise 50 5 0.2 \
      --out ${OUT}.prune
plink --bfile ${OUT}.step2b --extract ${OUT}.prune.prune.in \
      --genome --min 0.1875 \
      --out ${OUT}.ibd
...
plink --bfile ${OUT}.step2b --remove ${OUT}.ibdremove.txt --make-bed --out ${OUT}.step3
...
plink --bfile ${OUT}.step3 \
      --exclude high-LD-regions.txt \
      --indep-pairwise 50 5 0.2 \
      --out ${OUT}.step3.indep
plink --bfile ${OUT}.step3 --extract ${OUT}.step3.indep.prune.in \
...
plink --bfile ${OUT}.step3 --remove remove.outliers.txt --make-bed --out ${OUT}.step4

# ---------- Variant-level QC -------------------------------------------------

plink --bfile ${OUT}.step4 \
      --geno 0.05 \
      --maf 0.01 \
      --hwe 1e-5 \
      --make-bed --out ${OUT}.step5
# (HWE test performed in controls)
# Differential missingness between cases and controls (n=39,687 in the study)
plink --bfile ${OUT}.step5 \
      --test-missing \
      --out ${OUT}.missingdiff
awk 'NR==1 || $6 < 1e-5 {print $2}' ${OUT}.missingdiff.missing > ${OUT}.diffmiss.snps
plink --bfile ${OUT}.step5 --exclude ${OUT}.diffmiss.snps \
