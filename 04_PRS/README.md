...
| `PRS_prediction.R` | Regresses case-control status on the Z-score-standardised PRS by logistic regression in R (`glm()`, `family = binomial(logit)`), including the same covariates as in the MDD-Han GWAS. The variance explained (R²) is converted to Nagelkerke pseudo-R² with the `fmsb` package and then to liability-scaled Nagelkerke pseudo-R² assuming DEP prevalence of 5 %-20 %. |
